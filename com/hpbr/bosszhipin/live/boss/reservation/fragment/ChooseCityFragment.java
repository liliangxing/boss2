package com.hpbr.bosszhipin.live.boss.reservation.fragment;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;
import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import ba.i;
import com.hpbr.bosszhipin.base.BaseFragment;
import com.hpbr.bosszhipin.module.my.activity.boss.location.CitySelectFragment;
import com.hpbr.bosszhipin.module.my.activity.boss.location.g;
import com.hpbr.bosszhipin.module.my.entity.LevelBean;
import com.hpbr.bosszhipin.views.AppTitleView;
import java.util.ArrayList;
import java.util.List;
import xo.f;
import xo.h;

/* JADX INFO: loaded from: classes5.dex */
public class ChooseCityFragment extends BaseFragment {

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    private g f57784d;

    class a implements View.OnClickListener {
        a() {
        }

        @Override // android.view.View.OnClickListener
        public void onClick(View view) {
            ChooseCityFragment.this.Wf();
        }
    }

    class b implements g {
        b() {
        }

        @Override // com.hpbr.bosszhipin.module.my.activity.boss.location.g
        public void a(@NonNull LevelBean levelBean) {
            ChooseCityFragment.this.f57784d.a(levelBean);
            ChooseCityFragment.this.Wf();
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public void Wf() {
        if (getFragmentManager() != null) {
            getFragmentManager().popBackStack();
        }
    }

    public static ChooseCityFragment Xf(@NonNull List<LevelBean> list) {
        Bundle bundle = new Bundle();
        bundle.putSerializable("key_city", new ArrayList(list));
        ChooseCityFragment chooseCityFragment = new ChooseCityFragment();
        chooseCityFragment.setArguments(bundle);
        return chooseCityFragment;
    }

    @Override // androidx.fragment.app.Fragment
    @Nullable
    public View onCreateView(@NonNull LayoutInflater layoutInflater, @Nullable ViewGroup viewGroup, @Nullable Bundle bundle) {
        return layoutInflater.inflate(f.f146830p1, viewGroup, false);
    }

    @Override // androidx.fragment.app.Fragment
    public void onViewCreated(@NonNull View view, @Nullable Bundle bundle) {
        super.onViewCreated(view, bundle);
        AppTitleView appTitleView = (AppTitleView) view.findViewById(ba.g.Pu);
        appTitleView.setTitle(h.L0);
        appTitleView.z(i.O0, new a());
        appTitleView.A();
        if (getArguments() != null) {
            CitySelectFragment citySelectFragmentRg = CitySelectFragment.rg((List) getArguments().getSerializable("key_city"), true, 1);
            citySelectFragmentRg.setOnChooseCityListener(new b());
            getChildFragmentManager().beginTransaction().replace(xo.e.f146457u1, citySelectFragmentRg).commit();
        }
    }

    public void setOnChooseCityListener(g gVar) {
        this.f57784d = gVar;
    }
}